...
`docs/book/` — разбор кода от `main()` вниз, четырнадцать глав: что делает
каждая строчка, почему она такая и что сломается, если сделать иначе.
Начинать с `docs/book/00-index.md`. Незнакомые слова — в `13-glossary.md`.

Короткие справочные тексты:
